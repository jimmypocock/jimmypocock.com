...
add_todo() {
    ...
    local last_num=$(grep -oE '\[JPCOM-[0-9]{3}\]' "$TODO_FILE" | sed 's/\[JPCOM-//' | sed 's/\]//' | sort -n | tail -1)
    if [ -z "$last_num" ]; then
        last_num=0
    else
...
case "$1" in
    ...
    *)
        ...
        echo "                        Pattern can be a TODO key (e.g., JPCOM-001) or text"
        ...
        echo ""
        echo "Examples:"
        echo "  $0 complete JPCOM-001"
        echo "  $0 complete \"font size\""
        echo "  $0 progress JPCOM-002"
